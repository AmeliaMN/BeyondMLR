...
chart.long <- chart.wide |>
  pivot_longer(MathAvgScore.0:MathAvgScore.2, names_to = "key", values_to = "MathAvgScore") |>
  separate_wider_delim(key, names = c("name", "year08"), delim = ".") |>
  select(-c("...1", "name")) |>
...
regressions <- smallchart.long |>
  group_by(schoolid) |>
  do(tidy(lm(MathAvgScore ~ year08, data=.)))
...
lm_info <- smallchart.long |>
  group_by(schoolid) |>
  do(glance(lm(MathAvgScore ~ year08, data=.))) |>
  ungroup() |>
...
hgtwm <- score.nonm |>
  ...
  pivot_wider(names_from = newcol, values_from = lmres)
```
